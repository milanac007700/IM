...
public class KPSwitchRootLayoutHandler {
    ...
    public KPSwitchRootLayoutHandler(final View rootView) {
        ...
        mStatusBarHeight = StatusBarHeightUtil.getStatusBarHeight(rootView.getContext());
    ...
    public void handleBeforeMeasure(final int width, int height) {
        System.out.println("### RootLayoutHandler: handleBeforeMeasure()");
        //由于当前布局被键盘挤压，获知，由于键盘的活动，导致布局将要发生变化。

        if(mIsTranslucentStatus && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN
            && mTargetRootView.getFitsSystemWindows()) {
            //In this case, the height is always the same one, so, we have to calculate velow.
...
//        PullToRefreshListView.InternalListView listview = getListview(mTargetRootView);
        ...
        if(offset > 0) {
            //键盘弹起 (offset > 0，高度变小)
            System.out.println("### RootLayoutHandler: panel.handleHide()");
            panel.handleHide();
//            listview.handlePresssed(true);
            ...
            System.out.println("### RootLayoutHandler: panel.handleShow()");
            panel.handleShow();
//            listview.handlePresssed(true);
        }
    }
    ...
    private IPanelConflictLayout getPanelLayout(final View view) {
        ...
        if(view instanceof ViewGroup) {
            ...
            for (int i=0; i< viewGroup.getChildCount(); i++) {
                IPanelConflictLayout v = getPanelLayout(viewGroup.getChildAt(i));
                if(v != null) {
                    mPanelLayout = v;
                    return mPanelLayout;
                }
